...
def hexdump(src, length=16):
    ...
    for i in range(0, len(src), length):
        s = src[i:i+length]
        hexa = bytes(' ').join(["%0*X" % (digits, ord(x)) for x in s])
        text = bytes('').join([x if 0x20 <= ord(x) < 0x7F else bytes('.') for x in s])
        result.append(bytes("%04X  %-*s  %s") % (i, length*(digits +1), hexa, text))
...
def receive_from(connection):
    ...
    # We set a 2 secod timeout; depending on your target, this may need to be adjusted.
...
# Modify any requests destined for the local host.
